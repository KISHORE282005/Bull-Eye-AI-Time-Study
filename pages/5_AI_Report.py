...
✅ Total TOCT : **{round(df['toct'].sum(),2)} min**

✅ Total VA Time : **{overall['estimated_value_added_time']} sec**
""")
...
⚠ Total NVA : **{round(df['nva'].sum(),2)} min**

⚠ Repeat NVA : **{round(df['r_nva'].sum(),2)} min**

⚠ Walking Time : **{overall['walking_time']} sec**
""")
...
st.header("Management Remark")

st.success(f"""

The Industrial AI analysis identified **{len(df)} manufacturing processes**.

The estimated **Total Operation Cycle Time (TOCT)** is **{round(df['toct'].sum(),2)} minutes**.

The detected **Non-Value Added (NVA)** time is **{round(df['nva'].sum(),2)} minutes**.

The estimated **Repeat NVA** is **{round(df['r_nva'].sum(),2)} minutes**.

Recommended actions:

• Reduce operator walking.

• Improve workstation layout.

• Eliminate repeat NVA.

• Standardize process sequence.

• Implement Lean and 5S.
...
**Prepared for Bull Machines Pvt Ltd**
